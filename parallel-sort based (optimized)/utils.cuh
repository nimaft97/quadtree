...
__host__ __device__
void assignNumPoints(Point* points, Node anscestor, int* counts, int N, int tIdx, int bDim){
    int x_min, y_min, x_max, y_max, idx_s, idx_e;
        ...
        while(end + nodeNum*4 <= lengthMax){
            for (int i=start+blockIdx.x; i<end; i+=blockStride){
                ...
                if (node.mustSplit(K_MAX)){
                    ...
                    assignNumPoints(pShared, node, counts, pointNum, threadIdx.x, blockDim.x);
                    __syncthreads(); // until this point, each thread has its own counts array
